...
bool Resources::checkGameOver() {
    // 🔹 1. 자원이 0 이하이면 즉시 패배
    if (economy <= 0) {
        cout << "패배하였습니다! (국가 경제 체제가 붕괴되었습니다!)" << endl;
        return true;
    }
    if (military <= 0) {
        cout << "패배하였습니다! (국가의 군사력이 완전히 무너졌습니다!)" << endl;
        return true;
    }
    if (morale <= 0) {
        cout << "패배하였습니다! (국민들의 신뢰를 완전히 잃었습니다!)" << endl;
        return true;
    }
    if (religion <= 0) {
        cout << "패배하였습니다! (국가의 종교 기반이 완전히 사라졌습니다!)" << endl;
        return true;
    }
    if (scienceOrMagic <= 0) {
        if (nationType == "Science") {
            cout << "패배하였습니다! (과학이 몰락하여 지식이 사라졌습니다!)" << endl;
        }
        else {
            cout << "패배하였습니다! (마법이 사라져 왕국이 무너졌습니다!)" << endl;
        }
        return true;
    }
    ...
    for (int i = 0; i < 5; i++) {
        if (turnAtMax[i] >= 2) {
            switch (i) {
            case 0: cout << "패배하였습니다! (극단적 인플레이션으로 인해 국가가 붕괴되었습니다!)" << endl; break;
            case 1: cout << "패배하였습니다! (군사 독재가 발생하여 왕국이 무너졌습니다!)" << endl; break;
            case 2: cout << "패배하였습니다! (민심이 너무 높아 정부를 무시하는 폭동이 발생했습니다!)" << endl; break;
            case 3: cout << "패배하였습니다! (종교적 광신이 너무 강해 국가 운영이 불가능해졌습니다!)" << endl; break;
            case 4:
                if (nationType == "Science") {
                    cout << "패배하였습니다! (과학이 너무 발전하여 인공지능이 왕국을 지배했습니다!)" << endl;
                }
                else {
                    cout << "패배하였습니다! (마법이 너무 강해져 왕국이 현실을 초월해 사라졌습니다!)" << endl;
                }
                break;
            }
            return true;
